...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(
        ...
        for (UserMeal meal : meals) {
            sumCalories.merge(meal.getDateTime().toLocalDate(), meal.getCalories(),
                    (oldValue, newValue) ->
                            sumCalories.get(meal.getDateTime().toLocalDate()) + newValue
            );
        }
    ...
    public static List<UserMealWithExcess> filteredByStreams(
        ...
        Map<LocalDate, Integer> sumCalories = meals.stream()
                .collect(
                        Collectors.groupingBy(it -> it.getDateTime().toLocalDate(),
                                Collectors.summingInt(UserMeal::getCalories))
                );

        return meals.stream()
                .filter(it -> TimeUtil.isBetweenHalfOpen(it.getDateTime().toLocalTime(), startTime, endTime))
                .map(it -> createUserMealWithExcess(
                        it,
                        sumCalories.get(it.getDateTime().toLocalDate()) > caloriesPerDay
                ))
